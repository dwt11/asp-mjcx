...
function CheckOpenMenu()
{
	...
	var curitem = '';
	var curobj = null;
	var curobjsun = null;//131120加，原来的默认打开界面都是向下的箭头，无法自动获得状态
	//cross_obj = document.getElementById("staticbuttons");
	//setInterval("initializeIT()",20);
	
	if(ckstr==null)
	{
		ckstr='1_1';//如果第一次打开，默认第一个菜单是展开的
		setCookie('menuitems',ckstr,7);
	}
	ckstr = ','+ckstr+',';
	for(i=0;i<totalitem;i++)
	{
		...
		curobjsun = document.getElementById('sunitems'+curitem);//131120加，原来的默认打开界面都是向下的箭头，无法自动获得状态
		if(ckstr.indexOf(curitem) > 0 && curobj != null)
		{
			curobj.style.display = 'block';
			if(curobjsun != null)curobjsun.className = 'bitem';//131120加，原来的默认打开界面都是向下的箭头，无法自动获得状态
		}
		else
		{
			if(curobj != null) curobj.style.display = 'none';
            if(curobjsun != null)curobjsun.className = 'bitem2';//131120加，原来的默认打开界面都是向下的箭头，无法自动获得状态
		}
	}
}
