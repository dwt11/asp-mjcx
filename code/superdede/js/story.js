...
function DelCatalog(depid){
	if(window.confirm("你确实要删除这个分类么？"))
	{
		location.href='story_catalog.php?catid='+depid+'&action=del';
	}
}
...
function DelStoryContent(depid){
	if(window.confirm("删除内容后章节的其它内容排列序号不会发生变化，\r\n这可能导致管理混乱，你确实要删除这篇内容么？")){
		location.href='story_do.php?depid='+depid+'&action=delcontent';
	}
}
...
//预览内容
function PreViewCt(depid,booktype){
	if(booktype==0){
		window.open("../book/story.php?id="+depid);
	}else{
		window.open("../book/show-photo.php?id="+depid);
	}
}

//编辑部门
function EditCatalog(depid){
	$Obj('editCatalog').style.display='block';
	var myajax = new DedeAjax($Obj('editCatalogBody'),false,true,"","","请稍候，正在载入...");
	myajax.SendGet2('story_catalog.php?catid='+depid+'&action=editload');
	DedeXHTTP = null;
}
...
//删除整章节图书内容
function DelStoryChapter(depid){
	if(window.confirm("删除章节会删除章节下的所有内容，你确实要删除么？")){
		location.href='story_do.php?depid='+depid+'&action=delChapter';
	}
}
