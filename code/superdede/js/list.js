...
function moveArc(e, obj, depid){
	...
	LoadQuickDiv(e, 'archives_do.php?dopost=moveArchives&qstr='+qstr+'&channelid='+depid+'&rnd='+Math.random(), 'moveArchives', '450px', '180px');
	ChangeFullDiv('show');
}
function adArc(aid){
	var qstr=getCheckboxItem();
...
function ShowMenu(evt,obj,aid,atitle)
{
  ...
  //location="dep_main.php";
}
//获得选中文件的文件名
function getCheckboxItem()
{
	var allSel="";
	if(document.form2.ardepid.value) return document.form2.ardepid.value;
	for(i=0;i<document.form2.ardepid.length;i++)
	{
		if(document.form2.ardepid[i].checked)
		{
			if(allSel=="")
				allSel=document.form2.ardepid[i].value;
			else
				allSel=allSel+"`"+document.form2.ardepid[i].value;
		}
	}
	return allSel;
}
...
function selAll()
{
	for(i=0;i<document.form2.ardepid.length;i++)
	{
		if(!document.form2.ardepid[i].checked)
		{
			document.form2.ardepid[i].checked=true;
		}
	}
}
function noSelAll()
{
	for(i=0;i<document.form2.ardepid.length;i++)
	{
		if(document.form2.ardepid[i].checked)
		{
			document.form2.ardepid[i].checked=false;
		}
	}
}
-->