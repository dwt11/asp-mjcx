 ...
 * 部门操作
 *
 * @version        $Id: dep_do.php 1 14:31 12日Z tianya $
...
if(empty($dopost))
{
    ShowMsg("对不起，请指定部门参数！","dep_main.php");
    exit();
}
...
if($dopost=="addArchives")
{
    ...
    if($gurl=="")
    {
        ShowMsg("对不起，你指的部门可能有误！","dep_main.php");
        exit();
    }
...
else if($dopost=="upRankAll")
{
    ...
    UpDateCatCache();
    ShowMsg("操作成功，正在返回...","dep_main.php");
    exit();
}
/*--------------------------
//更新部门缓存
...
else if($dopost == 'unitCatalog')
{
    ...
    if(empty($nextjob))
    {
        ...
        if(!empty($row['dd']))
        {
            ShowMsg("部门： $typename($typeid) 有子部门，不能进行合并操作！", '-1');
            exit();
        }
        $typeOptions = $tl->GetOptionArray(0, 0, $channelid);
        $wintitle = '合并部门';
        $wecome_info = "<a href='dep_main.php'>部门管理</a> &gt;&gt; 合并部门";
        $win = new OxWindow();
        ...
        $win->AddTitle("合并目录时不会删除原来的部门目录，合并后需手动更新目标部门的文档HTML和列表HTML。");
        $win->AddItem('你选择的部门是：', "<font color='red'>$typename($typeid)</font>");
        $win->AddItem('你希望合并到那个部门？', "<select name='unittype'>\r\n{$typeOptions}\r\n</select>");
        $win->AddItem('注意事项：', '部门不能有下级子部门，只允许子级到更高级或同级或不同父级的情况。');
    ...
    else
    {
        if($typeid==$unittype)
        {
            ShowMsg("同一部门无法合并,请后退重试！", '-1');
            exit();
        }
        if(IsParent($unittype, $typeid))
        {
            ShowMsg('不能从父类合并到子类！', 'dep_main.php');
            exit();
        }
...
else if($dopost == 'moveCatalog')
{
    ...
    if(empty($nextjob))
    {
        ...
        $typeOptions = $tl->GetOptionArray(0,0,$channelid);
        $wintitle = "移动部门";
        $wecome_info = "<a href='dep_main.php'>部门管理</a> &gt;&gt; 移动部门";
        $win = new OxWindow();
        $win->Init('dep_do.php', 'js/blank.js', 'POST');
        ...
        $win->AddHidden('channelid', $channelid);
        $win->AddHidden('nextjob', 'unitok');
        $win->AddTitle("移动目录时不会删除原来已创建的列表，移动后需重新对部门创建HTML。");
        $win->AddItem('你选择的部门是：',"$typename($typeid)");
        $win->AddItem('你希望移动到那个部门？',"<select name='movetype'>\r\n<option value='0'>移动为顶级部门</option>\r\n$typeOptions\r\n</select>");
