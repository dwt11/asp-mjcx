 ...
 * 编辑系统管理员
 *
 * @version        $Id: sys_admin_user_edit.php 1 16:22 20日Z tianya $
...
if($dopost=='saveedit')
{
    ...
    if($id!=1){
        $query = "UPDATE `#@__sys_userid` SET uname='$uname',usertype='$usertype',tname='$tname',email='$email',typeid='$typeid' $pwd WHERE id='$id'";
    }else{
        $query = "UPDATE `#@__sys_userid` SET uname='$uname',tname='$tname',email='$email',typeid='$typeid' $pwd WHERE id='$id'";
    }
...
else if($dopost=='delete')
{
    ...
    $rs = $dsql->ExecuteNoneQuery2("DELETE FROM `#@__sys_userid` WHERE id='$id' AND id<>1 AND id<>'".$cuserLogin->getUserID()."' ");
    if($rs>0)
    {
        //更新前台用户信息
...
$safecode = substr(md5($cfg_cookie_encode.$randcode),0,24);
$typeOptions = '';
$row = $dsql->GetOne("SELECT * FROM `#@__sys_userid` WHERE id='$id'");
