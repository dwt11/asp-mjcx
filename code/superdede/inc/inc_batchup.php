 ...
 * 文档操作相关函数
 *
 * @version        $Id: inc_batchup.php 1 10:32 21日Z tianya $
    ...
    else
    {
        ...
        if(!$onlyfile)
        {
            ...
            if($dsql->ExecuteNoneQuery($query))
            {
                ...
                if($addtable != '')
                {
                    $dsql->ExecuteNoneQuery("Delete From `$addtable` where aid='$aid'");//7.3 根据论坛反馈，修复删除文章时无法清除附加表中对应的数据 (by：的鱼)
                }
                if($issystem != -1)
                {
                ...
                {
                    $dsql->Execute("me", "SELECT * FROM `#@__uploads` WHERE ardepid = '$aid'");
