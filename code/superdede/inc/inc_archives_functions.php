 ...
 * 文档操作相关函数
 *
 * @version        $Id: inc_archives_functions.php 1 9:56 21日Z tianya $
...
function GetCurContentAlbum($body, $rfurl, &$firstdd)
{
    ...
    foreach($img_array as $key=>$value)
    {
        ...
        if($rs)
        {
            ...
            //保存图片附件信息
            $inquery = "INSERT INTO `#@__uploads`(ardepid,title,url,mediatype,width,height,playtime,filesize,uptime,mid)
...
{
    global $cfg_multi_site,$cfg_basehost,$cfg_basedir,$cfg_image_dir,$ardepid,$cuserLogin,$dsql;
    ...
    foreach($img_array as $key=>$value)
    {
        ...
        if($rs)
        {
			...
			//保存图片附件信息
			$inquery = "INSERT INTO `#@__uploads`(ardepid,title,url,mediatype,width,height,playtime,filesize,uptime,mid)
			VALUES ('{$ardepid}','$rndFileName','$fileurl','1','{$imginfos[0]}','$imginfos[1]','0','$fsize','".time()."','".$cuserLogin->getUserID()."'); ";
			$dsql->ExecuteNoneQuery($inquery);
			$fid = $dsql->GetLastID();
...
{
    global $ardepid, $typeid, $cfg_make_andcat, $cfg_makeindex, $cfg_make_prenext;
    $revalue = $dolist = '';
    if($cfg_makeindex=='Y' || $cfg_make_andcat=='Y' || $cfg_make_prenext=='Y')
    {
        ...
        $jumpUrl = "task_do.php?typeid={$typeid}&aid={$ardepid}&dopost={$dolists[0]}&nextdo=".preg_replace("#".$dolists[0]."[,]{0,1}#", '', $dolist);
