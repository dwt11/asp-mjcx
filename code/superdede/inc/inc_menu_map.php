 ...
 * 菜单地图
 *
 * @version        $Id: inc_menu_map.php 1 10:32 21日Z tianya $
...
*///////////////////////////
  ...
  //载入主菜单
	...
	{
		$i++;
		$menusMain .= "<m:top mapitem='1' item='1_' name='".$row->plusname."' display='block'>";
		$menusMain .="  <m:item name='1111网666站部门管理' link='dep_main.php' ischannel='1' addalt='创建部门' linkadd='dep_add.php?listtype=all' rank='t_List,t_AccList' target='main' />
...
echo $GLOBALS['menusMain'];
/*
$menusMain = "
-----------------------------------------------

<m:top mapitem='1' item='1_' name='常用操作' display='block'>
  <m:item name='网站部门管理' link='dep_main.php' ischannel='1' addalt='创建部门' linkadd='dep_add.php?listtype=all' rank='t_List,t_AccList' target='main' />
  <m:item name='所有档案列表' link='content_list.php' rank='a_List,a_AccList' target='main' />
  <m:item name='等审核的档案' link='content_list.php?arcrank=-1' rank='a_Check,a_AccCheck' target='main' />
  <m:item name='我发布的文档' link='content_list.php?mid=".$cuserLogin->getUserID()."' rank='a_List,a_AccList,a_MyList' target='main' />
  <m:item name='评论管理' link='feedback_main.php' rank='sys_Feedback' target='main' />
...
<m:top mapitem='4' item='5_' name='HTML更新' notshowall='1' display='none' rank='sys_MakeHtml'>
  ...
  <m:item name='更新部门HTML' link='makehtml_list.php' rank='sys_MakeHtml' target='main' />
...
/*//载入插件菜单
$plusset = '';
$dsql->SetQuery("SELECT * FROM `#@__sys_left_class` WHERE isshow=1 ORDER BY aid ASC");
$dsql->Execute();
while($row = $dsql->GetObject()) 
{
