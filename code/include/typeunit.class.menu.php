 ...
 * 部门单元,主要用户管理后台管理菜单处
 *
 * @version        $Id: typeunit.class.menu.php 1 15:21 5日Z tianya $
 * @package        Libraries
...
class TypeUnit
{
    ...
    function __construct($catlogs='')
    {
        ...
        if(!empty($catlogs) && $catlogs!='-1')
        {
            ...
            foreach($this->aChannels as $depid)
            {
                if($cfg_Cs[$depid][0]==0)
                {
                    $this->dsql->SetQuery("Select id,ispart From `#@__arctype` where reid=$depid");
                    $this->dsql->Execute();
    ...
    function ListAllType($channel=0, $nowdir=0)
    {
        ...
        $this->dsql->Execute(0);
        $lastid = GetCookie('lastdepidMenu');
        while($row=$this->dsql->GetObject(0))
        {
            ...
            echo "  <dd class='dlr'><a href='dep_do.php?depid=".$id."&dopost=listArchives'{$smenu}>".$typeName."</a></dd>\r\n";
            echo "</dl>\r\n";
        ...
        if($this->dsql->GetTotalRow($fid)>0)
        {
            while($row=$this->dsql->GetObject($fid))
            {
                ...
                if(in_array($id,$this->aChannels) || $needcheck===false || $this->isAdminAll===true)
                {
                    ...
                    echo "   <tr>\r\n";
                    echo "     <td>".$step.$timg."<a href='dep_do.php?depid=".$id."&dopost=listArchives'{$smenu}>".$typeName."</a></td>\r\n";
                    echo "   </tr>\r\n";
                    echo "  </table>\r\n";
