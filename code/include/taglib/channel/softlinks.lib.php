 ...
 * 软件相关标签
 *
 * @version        $Id:softlinks.lib.php 1 9:33 8日Z tianya $
 * @package        Taglib
...
function ch_softlinks($fvalue, &$ctag, &$refObj, $fname='', $downloadpage=false)
{
    ...
    if($row['downtype']!='0' && !$downloadpage)
    {
        ...
        $links = $phppath."/download.php?open=0&aid=".$refObj->Ardepid."&depid=".$refObj->ChannelID;
...
function ch_softlinks_all($fvalue, &$ctag, &$refObj, &$row)
{
    ...
    foreach($dtp->CTags as $ctag)
    {
        if($ctag->GetName()=='link')
        {
            ...
            if($row['gotojump']==1)
            {
                $link = $phppath."/download.php?open=2&id={$refObj->Ardepid}&uhash={$uhash}";
            }
