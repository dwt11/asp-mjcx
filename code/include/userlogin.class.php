 ...
 *
 * @access    public
 * @param     int   $depid  频道id
 * @param     string   $msg  返回消息
 * @return    string
 */
function CheckCatalog($depid, $msg)
{
    ...
    if( !in_array($depid, $admin_catalogs) )
    {
...
function ClearMyAddon($aid=0, $title='')
{
    ...
    if(!empty($aid))
    {
        ...
        {
            if(!empty($title)) {
                $dsql->ExecuteNoneQuery("Update `#@__uploads` set ardepid='$aid',title='$title' where aid='{$addons[0]}'");
            }
            else {
                $dsql->ExecuteNoneQuery("Update `#@__uploads` set ardepid='$aid' where aid='{$addons[0]}' ");
            }
        }
    }
    @unlink($cacheFile);
...
class userLogin
{
    ...
    function checkUser($username, $userpwd)
    {
        ...
        $dsql->SetQuery("SELECT admin.* FROM `#@__admin` admin  WHERE admin.userid LIKE '".$this->userName."' LIMIT 0,1");
        $dsql->Execute();
        $row = $dsql->GetObject();
        if(!isset($row->pwd))
        {
