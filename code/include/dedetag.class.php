 ...
 * Dede模板类
 *
 * @version        $Id: dedetag.class.php 1 10:33 6日Z tianya $
 * @package        Libraries
...
class DedeTagParse
{
    ...
    function AssignSysTag()
    {
        ...
        for($i=0;$i<=$this->Count;$i++)
        {
            ...
            else if( $CTag->TagName == 'include' )
            {
                ...
                $str = $this->IncludeFile($filename,$CTag->GetAtt('ismake'),$CTag->GetAtt('iscode'));
                $this->CTags[$i]->IsReplace = TRUE;
                $this->CTags[$i]->TagValue = $str;
            }
     ...
     * @return    string
     */
    function IncludeFile($filename, $ismake='no',$iscode='no')
    {
       global $cfg_df_style;
      global $code_temp_path;  //130212添加，用于获取语句的模板路径
        ...
        if($iscode=="no")
        {
			...
			{
				return "无法在默认样式模板这个位置找到： $filename";
			}
		}
		else
		{
			...
			//dump($code_temp_path.$filename);
			if(file_exists($code_temp_path.$filename) )
			{
				$okfile = $code_temp_path.$filename;
			}
			else
			{
				return "";
